...
int StrategyBaseGridMAStopOpenCffex::MaxOnHandPositionCount = 0;
int StrategyBaseGridMAStopOpenCffex::OnHandPositionCount = 0;
...
StrategyBaseGridMAStopOpenCffex::StrategyBaseGridMAStopOpenCffex(char InstrumentID[30])
{
	iNextOrderRef = 1;
	mTickCount = 0;
	m_dOneTick = 0.01;
	...
	strcpy(mStrategyName, "BaseGridMAStopOpenCffex");
	strcpy(mInstanceName, "");
	WriteMsgToLogList("Strategy Init..");
	memset(curTradingDay, 0, sizeof(curTradingDay));
}

StrategyBaseGridMAStopOpenCffex::~StrategyBaseGridMAStopOpenCffex(void)
{
}

int StrategyBaseGridMAStopOpenCffex::GetShmindex()
{
	return shmindex;
}

void StrategyBaseGridMAStopOpenCffex::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

void StrategyBaseGridMAStopOpenCffex::SetInstanceName(char xInstanceName[50])
{
...
void StrategyBaseGridMAStopOpenCffex::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyBaseGridMAStopOpenCffex::SetParamValue(ParamNode node) {
	bool find = false;
	...
	else if (strcmp(node.ParamName, "yBuyPosition") == 0) {
		mStrategyParams.yBuyPosition = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "ySellPosition") == 0) {
		mStrategyParams.ySellPosition = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "OffFlagPoint") == 0) {
		mStrategyParams.OffFlagPoint = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "BuyLimitPrice") == 0) {
		mStrategyParams.BuyLimitPrice = atoi(node.ParamValue);
	}
	...
	else if (strcmp(node.ParamName, "IsOpenOrder") == 0) {
		if (atoi(node.ParamValue) >= 1)mStrategyParams.IsOpenOrder = true;
		else mStrategyParams.IsOpenOrder = false;
	}
...
void StrategyBaseGridMAStopOpenCffex::InitVariables() {
...
void StrategyBaseGridMAStopOpenCffex::ResetStrategy() {
}

void StrategyBaseGridMAStopOpenCffex::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
	...
	if (!mStrategyParams.IsBasePrice) {
		if (CloseOrderList.empty() && mOpenPrice - 0.00001 > 0) {
			mStrategyParams.BasePrice = mOpenPrice;
		}
	}
	...
	//	}
	//	// calc position
	//	//string strOpenOrderTime(closeorder_it2->OpenTime);
	//	//string strOpenDataDate=strOpenOrderTime.substr(0,strOpenOrderTime.find_first_of(" "));
	//	//if(0==strcmp(strOpenDataDate.c_str(),CTPTradingDay)) mTPosition++;
	//	//else mYPosition++;
	//	// End
	//}//end for Closed loop
	...
	//					if((newThostOrder.LimitPrice+0.00001)<=pDepthMarketData->lowerLimitPrice) return;
	//				}else if(MORDER_SELL==newThostOrder.Direction){
	//					if((newThostOrder.LimitPrice-0.00001)>=pDepthMarketData->upperLimitPrice) return;
	//				}
	...
	// 挂上网格
	if (nSellOrderType >= 30 && nSellOrderType <= (30 + mStrategyParams.UpGridCount - 1) && mStrategyParams.OpenSellAllow) {
		if (0 == GridCloseOrderCount[nSellOrderType]) {
			...
			// 开盘没持仓时进单
			bool bFlag = false;
			if (nHour_m == nOpenHour && nMin_m == nOpenMin && !mOrderRet && CloseOrderList.empty() && OpenOrderList.empty() && mStrategyParams.IsOpenOrder) {
				bFlag = true;
				mOrderRet = true;
			}
			else {
			...
			if (bOpenFlag && mStrategyParams.BasePrice - 0.00001 > 0
				...
				//int nCount=nSellOrderType-30+1;
				//openThostOrder.LimitPrice=mStrategyParams.BasePrice+mStrategyParams.OffFlagPoint*m_dOneTick+mStrategyParams.UpGridDistPoint*nCount*m_dOneTick;
				openThostOrder.LimitPrice = pDepthMarketData->lowerLimitPrice;
				// 判断价格是否在涨停
				if ((dPrice - 0.00001) >= pDepthMarketData->upperLimitPrice) return;
	...
	// 挂下一网格
	if (nBuyOrderType >= (29 - mStrategyParams.DnGridCount + 1) && nBuyOrderType >= 0 && nBuyOrderType <= 29 && mStrategyParams.OpenBuyAllow) {
		if (0 == GridCloseOrderCount[nBuyOrderType]) {
			...
			if (bOpenFlag && (mStrategyParams.BasePrice - 0.00001 > 0) && (mStrategyParams.BasePrice - 0.00001 < mStrategyParams.BuyLimitPrice)
				&& OnHandPositionCount <= MaxOnHandPositionCount && TotalOnHandPosition <= MaxTotalOnHandPosition
				&& bFlag) {
				MyOpenOrderType openThostOrder;
				iNextOrderRef++;
				...
				openThostOrder.OrderLocalRef = -1;
				//int nCount=29-nBuyOrderType+1;
				//openThostOrder.LimitPrice=mStrategyParams.BasePrice+mStrategyParams.OffFlagPoint*m_dOneTick-mStrategyParams.DnGridDistPoint*nCount*m_dOneTick;
				openThostOrder.LimitPrice = pDepthMarketData->upperLimitPrice;
				// 判断价格是否在跌停
				if ((dPrice + 0.00001) <= pDepthMarketData->lowerLimitPrice) return;
				openThostOrder.OrigSubmitPrice = openThostOrder.LimitPrice;
...
void StrategyBaseGridMAStopOpenCffex::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBaseGridMAStopOpenCffex::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
	...
	if (iter == matchnomap.end()) {
		...
		if (pOrderTradeMsg->OffFlag == THOST_FTDC_OF_Open && pOrderTradeMsg->Direction == THOST_FTDC_D_Buy) {
			string strIns(m_InstID);
			map<string, Posi>::iterator itor = gPosiMap.find(strIns);
			if (itor != gPosiMap.end()) itor->second.buyposition = 0;
		}
		else if (pOrderTradeMsg->OffFlag == THOST_FTDC_OF_Open && pOrderTradeMsg->Direction == THOST_FTDC_D_Sell) {
			string strIns(m_InstID);
			map<string, Posi>::iterator itor = gPosiMap.find(strIns);
			if (itor != gPosiMap.end()) itor->second.sellposition = 0;
		}
...
void StrategyBaseGridMAStopOpenCffex::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
void StrategyBaseGridMAStopOpenCffex::WideCharToMultiChar(CString str, char* dest_str)
{
...
bool StrategyBaseGridMAStopOpenCffex::timeRuleForOpen(char datadate[10], char datatime[10]) {
...
bool StrategyBaseGridMAStopOpenCffex::timeRuleForOpenNew(char datadate[10], char datatime[10]) {
	int nHour, nMin, nSec;
	sscanf(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
...
bool StrategyBaseGridMAStopOpenCffex::timeRuleForClose(char datadate[10], char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
bool StrategyBaseGridMAStopOpenCffex::timeRuleForCancel(char datadate[10], char datatime[10]) {
	int nYear, nMonth, nDate;
	nYear = atoi(datadate) / 10000;
	nMonth = (atoi(datadate) % 10000) / 100;
	nDate = (atoi(datadate) % 10000) % 100;
...
bool StrategyBaseGridMAStopOpenCffex::timeRuleForClosePrice(char datadate[10], char datatime[10]) {
	int nHour, nMin, nSec;
	sscanf_s(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
...
void StrategyBaseGridMAStopOpenCffex::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
	...
	if (ThostTraderAPI || SgitTraderAPI) {
		...
		string strIns(m_InstID);
		map<string, Posi>::iterator itor = gPosiMap.find(strIns);
		int nBuyPosition = 0, nSellPosition = 0;
		if (itor != gPosiMap.end()) {
			if (ThostTraderAPI)pThostTraderSpi->ReqOrderInsertCffex(&cOrder, m_bCrossTradingDay, false, shmindex, itor->second.buyposition, itor->second.sellposition);
			else pSgitTraderSpi->ReqOrderInsert(&cOrder, m_bCrossTradingDay, false, shmindex);
		}
		else {
			if (ThostTraderAPI)pThostTraderSpi->ReqOrderInsertCffex(&cOrder, m_bCrossTradingDay, false, shmindex, nBuyPosition, nSellPosition);
			else pSgitTraderSpi->ReqOrderInsert(&cOrder, m_bCrossTradingDay, false, shmindex);
		}
...
void StrategyBaseGridMAStopOpenCffex::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyBaseGridMAStopOpenCffex::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyBaseGridMAStopOpenCffex::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyBaseGridMAStopOpenCffex::OnDisplayLocalCloseOrderList()
{
	char log[200];
	string strIns(m_InstID);
	map<string, Posi>::iterator itor = gPosiMap.find(strIns);
	int nBuyPosition = 0, nSellPosition = 0;
	if (itor != gPosiMap.end()) {
		nBuyPosition = itor->second.buyposition;
		nSellPosition = itor->second.sellposition;
	}

	sprintf(log, "%s,开盘价=%.2f,基准价:%.2f,卖昨仓:%d,买昨仓:%d", mStrategyAndInstance.c_str(), mOpenPrice, mStrategyParams.BasePrice,
		nSellPosition, nBuyPosition);
	CString str(log);
...
void StrategyBaseGridMAStopOpenCffex::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyBaseGridMAStopOpenCffex::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyBaseGridMAStopOpenCffex::RecoverInstance(char cfgFileName[500])
{
...
void StrategyBaseGridMAStopOpenCffex::FlushStrategyInfoToFile() {
...
wstring StrategyBaseGridMAStopOpenCffex::s2ws(const string& s)
{
...
int StrategyBaseGridMAStopOpenCffex::CreateStrategyMapOfView() {
	CString strPathFile;
...
int StrategyBaseGridMAStopOpenCffex::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyBaseGridMAStopOpenCffex::CloseMap()
{
	//删除对应的cfg文件
...
void StrategyBaseGridMAStopOpenCffex::SetCloseLocalOrder(LocalCLForDisplayField* pCLOrder)
{
}

void StrategyBaseGridMAStopOpenCffex::SetStrategyID(char strategyId[50])
{
	strcpy(mStrategyID, strategyId);
}

void StrategyBaseGridMAStopOpenCffex::InitAction() {
	mTickCount = 0;
	m_bSampleReady = false;
	mOpenRet = false;
	InitVariables();
...
void StrategyBaseGridMAStopOpenCffex::WriteBasePrice(char* date, char* time) {
	char line[200];
