...
#include <map>
using namespace std;
//#define ES_GRIDCOUNT 120		// 最大网格数量
// 开仓和平仓撤单后不会重新挂单

class StrategyBaseGridOpen_plus : public CStrategy
{
	struct mParasType
	{
...
private:
	...
	double mOpenPrice;
public:
	StrategyBaseGridOpen_plus(char InstrumentID[30]);
	~StrategyBaseGridOpen_plus(void);
	...
	int mTickCount;
	int GridCloseOrderCount[ES_GRIDCOUNT];
