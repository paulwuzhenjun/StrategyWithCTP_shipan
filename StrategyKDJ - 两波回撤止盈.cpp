...
int StrategyKDJ::MaxOnHandPositionCount=0;
int StrategyKDJ::OnHandPositionCount=0;
...
StrategyKDJ::StrategyKDJ(char InstrumentID[30]):m_Price(0)
{					
	...
	RSVBuffer=(double*)malloc(sizeof(double)*m_dallocBarNum);
	KBuffer=(double*)malloc(sizeof(double)*m_dallocBarNum);
	DBuffer=(double*)malloc(sizeof(double)*m_dallocBarNum);
	JBuffer=(double*)malloc(sizeof(double)*m_dallocBarNum);
...
int StrategyKDJ::GetShmindex()
{
	return shmindex;
}

void StrategyKDJ::SetShmindex(int xshmindex)
{
	shmindex=xshmindex;
}

wstring StrategyKDJ::s2ws(const string& s)
{
...
int StrategyKDJ::CreateStrategyMapOfView(){
...
int StrategyKDJ::OpenStrategyMapOfView(){
...
void StrategyKDJ::SetInstanceName(char xInstanceName[50])
{
...
void StrategyKDJ::GetInstanceName(char *xInstanceName){
	strcpy(xInstanceName,mInstanceName);
}

void StrategyKDJ::SetParamValue(ParamNode node){
	bool find=false;
	...
	}else if(strcmp(node.ParamName,"NPeriod")==0){
		mStrategyParams.NPeriod=atoi(node.ParamValue);
	}else if(strcmp(node.ParamName,"KPeriod")==0){
		mStrategyParams.KPeriod=atoi(node.ParamValue);
	}else if(strcmp(node.ParamName,"DPeriod")==0){
		mStrategyParams.DPeriod=atoi(node.ParamValue);
	}else if(strcmp(node.ParamName,"JPeriod")==0){
		mStrategyParams.JPeriod=atoi(node.ParamValue);
	}else if(strcmp(node.ParamName,"ProfitPoint")==0){
		mStrategyParams.ProfitPoint=atof(node.ParamValue);
	}else if(strcmp(node.ParamName,"MAProfitPoint")==0){
		mStrategyParams.MAProfitPoint=atof(node.ParamValue);
	}else if(strcmp(node.ParamName,"BackPerc")==0){
		mStrategyParams.BackPerc=atof(node.ParamValue);
	}else if(strcmp(node.ParamName,"MAProfitPoint2")==0){
		mStrategyParams.MAProfitPoint2=atof(node.ParamValue);
	}else if(strcmp(node.ParamName,"BackPerc2")==0){
		mStrategyParams.BackPerc2=atof(node.ParamValue);
...
void StrategyKDJ::InitVariables(){
...
void StrategyKDJ::ResetStrategy(){

}
void StrategyKDJ::ResetAction(){
	char line[200];
...
/*
void StrategyKDJ::MessageProcess()
{
...
*/
void StrategyKDJ::OnRtnDepthMarketData(TickInfo *pDepthMarketData)
{
	...
	//if(curTickTime>tmt_StartTime) {
		...
		mIndicator.iKDJ(mBarFile,mBarIndex,mBarIndex+1,14,3,3,3,RSVBuffer,KBuffer,DBuffer,JBuffer);
		...
		if(lastmBarIndex!=mBarIndex&&mBarIndex>0&&mTodayTickCount>2
			...
			&&strcmp(mBarFile[mBarIndex-1].datatime,"11:30:00")!=0){
				...
				//sprintf(dataline,"%s,%s,%.4f,%.4f,%.4f,%.4f\n",mBarFile[mBarIndex-1].datadate,mBarFile[mBarIndex-1].datatime,RSVBuffer[mBarIndex-1],KBuffer[mBarIndex-1],DBuffer[mBarIndex-1],JBuffer[mBarIndex-1]);
	...
	if(KBuffer[mBarIndex-2]<DBuffer[mBarIndex-2]&&KBuffer[mBarIndex-1]>DBuffer[mBarIndex-1]
	//&&KBuffer[mBarIndex-1]<45
	){
		EnterBuyTrade=1;
	}
		...
		for(closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();){			
			...
			if( iter_e->Direction==MORDER_SELL&&
				(iter_e->OrderStatus==MORDER_PART_TRADED||iter_e->OrderStatus==MORDER_ACCEPTED||iter_e->OrderStatus==MORDER_QUEUED)){
					...
					if((iter_e->maxProfit+0.00001)>mStrategyParams.MAProfitPoint2*m_dOneTick){
						iter_e->mStoplossPoint=floor(-iter_e->maxProfit*(1.0-mStrategyParams.BackPerc2)/m_dOneTick+0.00001);
					}else if((iter_e->maxProfit+0.00001)>mStrategyParams.MAProfitPoint*m_dOneTick){
						iter_e->mStoplossPoint=floor(-iter_e->maxProfit*(1.0-mStrategyParams.BackPerc)/m_dOneTick+0.00001);
					}else if((iter_e->maxProfit-0.00001)<mStrategyParams.MAProfitPoint*m_dOneTick){
					...
					if( iter_e->IsClosePofitOrder&&(iter_e->OpenOrderTradePrice-m_Price)>=(iter_e->mStoplossPoint*m_dOneTick-0.00001)						
						//&&timeRuleForClose(pDepthMarketData->datadate,pDepthMarketData->updatetime)
							...
							char line[200];
							sprintf(line,"%s,Buy Order Stoploss Close,%.4f,%.1f,%.4f,OrderId=%d",mStrategyAndInstance.c_str(),iter_e->OpenOrderTradePrice,iter_e->mStoplossPoint,m_Price,iter_e->OrderId);
							WriteMsgToLogList(line);
				...
				(iter_e->OrderStatus==MORDER_PART_TRADED||iter_e->OrderStatus==MORDER_ACCEPTED||iter_e->OrderStatus==MORDER_QUEUED)){
					...
					if( iter_e->IsClosePofitOrder&&(m_Price-iter_e->OpenOrderTradePrice)>=(iter_e->mStoplossPoint*m_dOneTick-0.00001)						
						//&&timeRuleForClose(pDepthMarketData->datadate,pDepthMarketData->updatetime)
							...
							char line[200];
							sprintf(line,"%s,Sell Order Stoploss Close,%.4f,%.1f,%.4f,OrderId=%d",mStrategyAndInstance.c_str(),iter_e->OpenOrderTradePrice,iter_e->mStoplossPoint,m_Price,iter_e->OrderId);
							WriteMsgToLogList(line);
...
void StrategyKDJ::OnRtnOrder(OrderTradeMsg *pOrderTradeMsg)
{ 
...
void StrategyKDJ::FlushStrategyInfoToFile(){
...
void StrategyKDJ::OnRtnTrade(OrderTradeMsg *pOrderTradeMsg)
{
...
void StrategyKDJ::OnRspOrderInsert(ShmRspOrderInsert *pRspOrderInsert)
{
...
bool StrategyKDJ::timeRuleForOpen(char datadate[10],char datatime[10]){
...
bool StrategyKDJ::timeRuleForClose(char datadate[10],char datatime[10]){
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
bool StrategyKDJ::timeRuleOK(char datatime[10]){
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
void StrategyKDJ::ReqOpenOrderInsert(MyOpenOrderType *pOpenOrder){
...
void StrategyKDJ::ReqCloseOrderInsert(MyCloseOrderType *pCloseOrder,char OpenOrderTime[21]){
...
void StrategyKDJ::ReqOrderDelete(int pOrderId,int pOrderLocalRef,int pFrontID,int pSessionID)
{
	if(ThostTraderAPI||SgitTraderAPI){
...
void StrategyKDJ::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyKDJ::OnDisplayLocalCloseOrderList()
{
	std::list<MyOpenOrderType>::iterator openorder_it;
	for(openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end();openorder_it++){
	...
	if(mBarIndex>0){
	char log[200];
	sprintf(log,"%s,%d,K=%.2f,D=%.2f",mStrategyAndInstance.c_str(),CloseOrderList.size(),KBuffer[mBarIndex],DBuffer[mBarIndex]);
	CString str(log);
	pPubMsg->AddString(str);
	}
}

void StrategyKDJ::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyKDJ::DisplayTradeOnScreen(OrderTradeMsg *pOrderTradeMsg,int mDirection,int mOpenOrClose,int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE); 
...
void StrategyKDJ::RecoverInstance(char cfgFileName[500])
{
...
void StrategyKDJ::AddToRatesX(int tickCount,TickInfo *curTick,int gRatesXSize){
...
int StrategyKDJ::initHstDataFromCSV(){
...
void StrategyKDJ::AddInToDesRatesX(BarRateInfo* gCurrtRatesX, int gSourceRatesXIndex,BarRateInfo* gDesRatesX,int *gDesRatesXIndex,int gDesRatesXSize){
...
void StrategyKDJ::SetCloseLocalOrder(LocalCLForDisplayField *pCLOrder)
{
...
void StrategyKDJ::SetStrategyID(char strategyId[50])
{
