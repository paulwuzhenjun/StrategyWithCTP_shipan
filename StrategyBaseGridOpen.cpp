...
int StrategyBaseGridOpen::MaxOnHandPositionCount = 0;
int StrategyBaseGridOpen::OnHandPositionCount = 0;
...
StrategyBaseGridOpen::StrategyBaseGridOpen(char InstrumentID[30])
{
	iNextOrderRef = 1;
	mTickCount = 0;
	m_dOneTick = 0.01;
	...
	strcpy(mStrategyName, "BaseGridOpen");
	strcpy(mInstanceName, "");
	WriteMsgToLogList("Strategy Init..");
	memset(curTradingDay, 0, sizeof(curTradingDay));
}

StrategyBaseGridOpen::~StrategyBaseGridOpen(void)
{
}

int StrategyBaseGridOpen::GetShmindex()
{
	return shmindex;
}

void StrategyBaseGridOpen::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

void StrategyBaseGridOpen::SetInstanceName(char xInstanceName[50])
{
...
void StrategyBaseGridOpen::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyBaseGridOpen::SetParamValue(ParamNode node) {
	bool find = false;
...
void StrategyBaseGridOpen::InitVariables() {
...
void StrategyBaseGridOpen::ResetStrategy() {
}

void StrategyBaseGridOpen::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
	...
	if (!CloseOrderList.empty()) {
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();) {
			...
			if (iter_e->Direction == MORDER_SELL
				...
				if (iter_e->IsClosePofitOrder && (m_Price - iter_e->OpenOrderTradePrice + 0.00001 > mStrategyParams.ProfitPoint* m_dOneTick/*m_Price+0.00001>iter_e->ProfitPrice*/)) {
					char line[200];
				...
				&& (iter_e->OrderStatus == MORDER_PART_TRADED || iter_e->OrderStatus == MORDER_ACCEPTED || iter_e->OrderStatus == MORDER_QUEUED)) {
				//Closing sell order
				if (iter_e->IsClosePofitOrder && (iter_e->OpenOrderTradePrice - m_Price + 0.00001 > mStrategyParams.ProfitPoint* m_dOneTick/*m_Price-0.00001<iter_e->ProfitPrice*/)) {
					char line[200];
	...
	if (nSellOrderType >= 30 && nSellOrderType <= (30 + mStrategyParams.UpGridCount - 1)) {
		...
		if (0 == GridCloseOrderCount[nSellOrderType] && (m_Price + 0.00001 > dGridPrice)) {
			if (bOpenFlag && mStrategyParams.OpenSellAllow && mStrategyParams.BasePrice - 0.00001 > 0
				...
				openThostOrder.LimitPrice = pDepthMarketData->lowerLimitPrice;	// 市价
				// 判断价格是否在涨停
				if ((m_Buy1 + 0.00001) > pDepthMarketData->upperLimitPrice) return;
				openThostOrder.OrigSubmitPrice = dGridPrice;
				...
				openThostOrder.ProfitPrice = dGridPrice - mStrategyParams.ProfitPoint * m_dOneTick;
	...
	if (nBuyOrderType >= (29 - mStrategyParams.DnGridCount + 1) && nBuyOrderType >= 0 && nBuyOrderType <= 29) {
		...
		if (0 == GridCloseOrderCount[nBuyOrderType] && (m_Price - 0.00001 < dGridPrice)) {
...
void StrategyBaseGridOpen::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end(); closeorder_it++) {
			...
			if (closeorder_it->OrderId == thisOrderId || (closeorder_it->OrderLocalRef == pOrderTradeMsg->OrderLocalRef && closeorder_it->OrderLocalRef > 0)) {
				...
				if (pOrderTradeMsg->OrderStatus == MORDER_PART_CANCELLED || pOrderTradeMsg->OrderStatus == MORDER_CANCELLED) {
					...
					//	char linex[200];
					//	sprintf(linex,"%s,OnRtnOrder IsStoplessOrder||IsClosePofitOrder,Ref=%d",mStrategyAndInstance.c_str(),pOrderTradeMsg->OrderLocalRef);
					...
					//		newThostOrder.LimitPrice=m_Buy1-10*m_dOneTick;
					...
					//		newThostOrder.MOrderType=closeorder_it->MOrderType;
					//		newThostOrder.Direction=closeorder_it->Direction;
					//		newThostOrder.LimitPrice=m_Sell1+10*m_dOneTick;
					//		newThostOrder.OrigSubmitPrice=closeorder_it->OrigSubmitPrice;
					//		newThostOrder.Offset=closeorder_it->Offset;
					//		newThostOrder.VolumeTotalOriginal=closeorder_it->VolumeTotal;
					//		newThostOrder.VolumeTotal=newThostOrder.VolumeTotalOriginal;
...
void StrategyBaseGridOpen::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBaseGridOpen::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
void StrategyBaseGridOpen::WideCharToMultiChar(CString str, char* dest_str)
{
...
bool StrategyBaseGridOpen::timeRuleForOpen(char datadate[10], char datatime[10]) {
...
bool StrategyBaseGridOpen::timeRuleForOpenNew(char datadate[10], char datatime[10]) {
	int nHour, nMin, nSec;
	sscanf(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
...
bool StrategyBaseGridOpen::timeRuleForClose(char datadate[10], char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
bool StrategyBaseGridOpen::timeRuleForCancel(char datadate[10], char datatime[10]) {
	int nYear, nMonth, nDate;
	nYear = atoi(datadate) / 10000;
	nMonth = (atoi(datadate) % 10000) / 100;
	nDate = (atoi(datadate) % 10000) % 100;
...
bool StrategyBaseGridOpen::timeRuleForClosePrice(char datadate[10], char datatime[10]) {
	int nHour, nMin, nSec;
	sscanf_s(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
...
void StrategyBaseGridOpen::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyBaseGridOpen::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.ExchangeID, m_ExchangeID);
...
void StrategyBaseGridOpen::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyBaseGridOpen::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyBaseGridOpen::OnDisplayLocalCloseOrderList()
{
	char log[200];
	sprintf(log, "%s,策略开盘价=%.5f,基准价=%.4f,收盘时间:%s", mStrategyAndInstance.c_str(), mOpenPrice,
...
void StrategyBaseGridOpen::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyBaseGridOpen::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyBaseGridOpen::RecoverInstance(char cfgFileName[500])
{
...
void StrategyBaseGridOpen::FlushStrategyInfoToFile() {
...
wstring StrategyBaseGridOpen::s2ws(const string& s)
{
...
int StrategyBaseGridOpen::CreateStrategyMapOfView() {
	CString strPathFile;
...
int StrategyBaseGridOpen::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyBaseGridOpen::CloseMap()
{
	//删除对应的cfg文件
...
void StrategyBaseGridOpen::SetCloseLocalOrder(LocalCLForDisplayField* pCLOrder)
{
}

void StrategyBaseGridOpen::SetStrategyID(char strategyId[50])
{
	strcpy(mStrategyID, strategyId);
}

void StrategyBaseGridOpen::InitAction() {
	mTickCount = 0;
	m_bSampleReady = false;
	mOpenRet = false;
	InitVariables();
...
void StrategyBaseGridOpen::WriteBasePrice(char* date, char* time) {
	char line[200];
