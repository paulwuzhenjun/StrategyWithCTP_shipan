...
class StrategyBaseGridMAStopOpenCffex : public CStrategy
{
	struct mParasType
	{
		...
		int yBuyPosition;
		int ySellPosition;
		int OffFlagPoint;				// 基准价偏移
		double BuyLimitPrice;
		double BackPerc;				// 回撤百分比
		bool IsOpenOrder;				// 是否开盘进单
	};
...
private:
	...
	bool mOrderRet;
public:
	StrategyBaseGridMAStopOpenCffex(char InstrumentID[30]);
	~StrategyBaseGridMAStopOpenCffex(void);
