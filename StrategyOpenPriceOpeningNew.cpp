...
int StrategyOpenPriceOpeningNew::MaxOnHandPositionCount = 0;
int StrategyOpenPriceOpeningNew::OnHandPositionCount = 0;
...
StrategyOpenPriceOpeningNew::StrategyOpenPriceOpeningNew(char InstrumentID[30]) :m_Price(0)
{
	...
	*/
	...
	strcpy(mStrategyName, "OpenPriceOpeningNew");
...
void StrategyOpenPriceOpeningNew::CloseMap()
{
	//删除对应的cfg文件
...
StrategyOpenPriceOpeningNew::~StrategyOpenPriceOpeningNew(void)
{
}

int StrategyOpenPriceOpeningNew::GetShmindex()
{
	return shmindex;
}

void StrategyOpenPriceOpeningNew::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

wstring StrategyOpenPriceOpeningNew::s2ws(const string& s)
{
...
int StrategyOpenPriceOpeningNew::CreateStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
int StrategyOpenPriceOpeningNew::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyOpenPriceOpeningNew::SetInstanceName(char xInstanceName[50])
{
...
void StrategyOpenPriceOpeningNew::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyOpenPriceOpeningNew::SetParamValue(ParamNode node) {
	bool find = false;
...
void StrategyOpenPriceOpeningNew::InitVariables() {
...
void StrategyOpenPriceOpeningNew::InitAction() {
	mOpenTimes = 0;
	Type1OrderExist = false;
	Type2OrderExist = false;
	Type2StoplossClose = false;
...
void StrategyOpenPriceOpeningNew::ResetStrategy() {
}
//检查消息队列，并将得到的消息转给相应的处理函数
/*
void StrategyOpenPriceOpeningNew::MessageProcess()
{
...
*/
void StrategyOpenPriceOpeningNew::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
	...
	if (timeRuleForOpen(pDepthMarketData->datadate, pDepthMarketData->updatetime) && !Type1OrderExist && !m_bType1StoplossClose && !Type2StoplossClose) {
...
void StrategyOpenPriceOpeningNew::CheckAndResubmitOutOfDateOrder(char cTickDataDate[11], char cTickUpdateTime[11]) {
...
void StrategyOpenPriceOpeningNew::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyOpenPriceOpeningNew::FlushStrategyInfoToFile() {
...
void StrategyOpenPriceOpeningNew::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
			...
			for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); openorder_it++) {
				...
				if (openorder_it->OrderId == thisTradeRef || (openorder_it->OrderLocalRef == pOrderTradeMsg->OrderLocalRef && openorder_it->OrderLocalRef > 0 && (ThostTraderAPI || SgitTraderAPI))) {
					...
					if (openorder_it->Direction == MORDER_BUY)
					{
						...
						//报第二个开仓单到交易所
						if (openorder_it->MOrderType == 1 && mOpenTimes < mStrategyParams.LoopTimes
							&& mOpenPrice>0.00001
							&& !mType2OrderExist
...
void StrategyOpenPriceOpeningNew::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
bool StrategyOpenPriceOpeningNew::timeRuleForOpen(char datadate[10], char datatime[10]) {
...
bool StrategyOpenPriceOpeningNew::timeRuleForClose(char datadate[10], char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
void StrategyOpenPriceOpeningNew::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyOpenPriceOpeningNew::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyOpenPriceOpeningNew::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyOpenPriceOpeningNew::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyOpenPriceOpeningNew::OnDisplayLocalCloseOrderList()
{
	char log[200];
...
void StrategyOpenPriceOpeningNew::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyOpenPriceOpeningNew::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyOpenPriceOpeningNew::RecoverInstance(char cfgFileName[500])
{
...
void StrategyOpenPriceOpeningNew::SetStrategyID(char strategyId[50])
{
