...
int StrategyBandMomnt::MaxOnHandPositionCount = 0;
int StrategyBandMomnt::OnHandPositionCount = 0;
...
StrategyBandMomnt::StrategyBandMomnt(char InstrumentID[30]) :m_Price(0)
{
	...
	strcpy(mStrategyName, "BandMomnt");
...
void StrategyBandMomnt::CloseMap()
{
	//删除对应的cfg文件
...
StrategyBandMomnt::~StrategyBandMomnt(void)
{
	free(mBarFile);
...
int StrategyBandMomnt::GetShmindex()
{
	return shmindex;
}

void StrategyBandMomnt::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

wstring StrategyBandMomnt::s2ws(const string& s)
{
...
int StrategyBandMomnt::CreateStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
int StrategyBandMomnt::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyBandMomnt::SetInstanceName(char xInstanceName[50])
{
...
void StrategyBandMomnt::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyBandMomnt::SetParamValue(ParamNode node) {
	bool find = false;
	...
	else if (strcmp(node.ParamName, "BandsPeriod") == 0) {
		mStrategyParams.BandsPeriod = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "BandsDeviations") == 0) {
		mStrategyParams.BandsDeviations = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "HLDist") == 0) {
		mStrategyParams.HLDist = atof(node.ParamValue);
	}
...
void StrategyBandMomnt::InitVariables() {
	...
	cerr << strmdfilename.c_str() << endl;
	fmddata = fopen(strmdfilename.c_str(), "a+");
}
void StrategyBandMomnt::ResetStrategy() {
}

void StrategyBandMomnt::ResetAction() {
	char line[200];
...
/*
void StrategyBandMomnt::MessageProcess()
{
...
*/
void StrategyBandMomnt::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
...
void StrategyBandMomnt::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBandMomnt::FlushStrategyInfoToFile() {
...
void StrategyBandMomnt::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBandMomnt::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
bool StrategyBandMomnt::timeRuleForOpen(char datadate[10], char datatime[10]) {
...
bool StrategyBandMomnt::timeRuleForClose(char datadate[10], char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
void StrategyBandMomnt::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyBandMomnt::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyBandMomnt::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyBandMomnt::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyBandMomnt::OnDisplayLocalCloseOrderList()
{
	std::list<MyOpenOrderType>::iterator openorder_it;
	for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); openorder_it++) {
...
void StrategyBandMomnt::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyBandMomnt::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyBandMomnt::RecoverInstance(char cfgFileName[500])
{
...
void StrategyBandMomnt::AddToRatesX(int tickCount, TickInfo* curTick, int gRatesXSize) {
	int nYear, nMonth, nDate, nHour, nMin, nSec;
...
int StrategyBandMomnt::initHstDataFromCSV() {
...
void StrategyBandMomnt::AddInToDesRatesX(BarRateInfo* gCurrtRatesX, int gSourceRatesXIndex, BarRateInfo* gDesRatesX, int* gDesRatesXIndex, int gDesRatesXSize) {
...
bool StrategyBandMomnt::timeRuleOK(char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
void StrategyBandMomnt::SetStrategyID(char strategyId[50])
{
