...
int StrategyDT::MaxOnHandPositionCount = 0;
int StrategyDT::OnHandPositionCount = 0;

StrategyDT::StrategyDT(char InstrumentID[30])
{
...
StrategyDT::~StrategyDT(void)
{
}

void StrategyDT::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	...
	// 获取开盘价格
	if (!bOpenRet) {
		if (nHour_m == 9 && nMin_m >= nOpenMin) {
			mOpenPrice = pDepthMarketData->price;
			bOpenRet = true;
			//LoadHisData(mBarFile);
		}
	}
	...
	char opentime[] = "09:05:00";
	char opentime_end[] = "02:30:00";
	if (m_Price - 0.00001 > (mOpenPrice + DT) && DT - 0.00001 > 0 && mBarFile[mBarIndex - 1].close > MA2Avg
		&& MA2Avg > PreMA2Avg&& MA1Avg > MA2Avg && (strcmp(pDepthMarketData->updatetime, opentime) > 0 || strcmp(pDepthMarketData->updatetime, opentime_end) < 0)
		&& 0 == mOpenTimes) {
		EnterBuyTrade = 1;
		char line[200];
	...
	if (!CloseOrderList.empty()) {
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();) {
			...
			if (iter_e->Direction == MORDER_SELL &&
				...
				if ((iter_e->maxProfit + 0.00001) > iter_e->OpenOrderTradePrice* mStrategyParams.ProfitPoint1) {
					iter_e->MAStop = true;
				}
				if (iter_e->IsClosePofitOrder && ((iter_e->maxProfit + 0.00001) < iter_e->OpenOrderTradePrice * mStrategyParams.ProfitPoint2) && m_Price < ((iter_e->OpenOrderTradePrice + iter_e->maxProfit - iter_e->OpenOrderTradePrice * mStrategyParams.ProfitPoint1) + 0.00001) && iter_e->MAStop) {
					char line[200];
					sprintf(line, "%s,Sell Order Signal 1,%.4f,%.4f,Ref=%d", mStrategyAndInstance.c_str(), iter_e->OpenOrderTradePrice, mBarFile[mBarIndex - 1].close, iter_e->OrderLocalRef);
					WriteMsgToLogList(line);
					...
					newThostOrder.LimitPrice = pDepthMarketData->lowerLimitPrice;
				...
				else if (iter_e->IsClosePofitOrder && ((iter_e->maxProfit + 0.00001) > iter_e->OpenOrderTradePrice* mStrategyParams.ProfitPoint2) && m_Price < ((iter_e->OpenOrderTradePrice + iter_e->maxProfit - iter_e->OpenOrderTradePrice * mStrategyParams.StoplossPoint2) + 0.00001) && iter_e->MAStop) {
					char line[200];
					sprintf(line, "%s,Sell Order Signal 2,%.4f,%.1f,Ref=%d", mStrategyAndInstance.c_str(), iter_e->OpenOrderTradePrice, mBarFile[mBarIndex - 1].close, iter_e->OrderLocalRef);
					WriteMsgToLogList(line);
				...
				else if (m_Price < iter_e->OpenOrderTradePrice * (1 - mStrategyParams.StoplossPoint)) {
					char line[200];
					sprintf(line, "%s,Buy Order Stop 0,%.4f,%.4f,%.4f,Ref=%d", mStrategyAndInstance.c_str(), iter_e->OpenOrderTradePrice, iter_e->mStoplossPoint, m_Price, iter_e->OrderLocalRef);
					WriteMsgToLogList(line);
					...
					newThostOrder.MOrderType = iter_e->MOrderType;
					newThostOrder.Direction = iter_e->Direction;
					newThostOrder.LimitPrice = pDepthMarketData->upperLimitPrice;
					newThostOrder.OrigSubmitPrice = newThostOrder.LimitPrice;
					newThostOrder.Offset = iter_e->Offset;
					newThostOrder.VolumeTotalOriginal = iter_e->VolumeTotal;
					newThostOrder.VolumeTotal = newThostOrder.VolumeTotalOriginal;
				...
				if (iter_e->IsClosePofitOrder && ((iter_e->maxProfit + 0.00001) < iter_e->OpenOrderTradePrice * mStrategyParams.ProfitPoint2) && m_Price > ((iter_e->OpenOrderTradePrice - iter_e->maxProfit) * (1 + mStrategyParams.StoplossPoint1) + 0.00001) && iter_e->MAStop) {
					char line[200];
					sprintf(line, "%s,Buy Order Signal 1,%.4f,%.4f,Ref=%d", mStrategyAndInstance.c_str(), iter_e->OpenOrderTradePrice, iter_e->mStoplossPoint, m_Price, iter_e->OrderLocalRef);
					WriteMsgToLogList(line);
				...
				else if (iter_e->IsClosePofitOrder && ((iter_e->maxProfit + 0.00001) > iter_e->OpenOrderTradePrice* mStrategyParams.ProfitPoint2) && m_Price > ((iter_e->OpenOrderTradePrice - iter_e->maxProfit) * (1 + mStrategyParams.StoplossPoint2) + 0.00001) && iter_e->MAStop) {
					char line[200];
					sprintf(line, "%s,Buy Order Signal 2,%.4f,%.4f,Ref=%d", mStrategyAndInstance.c_str(), iter_e->OpenOrderTradePrice, iter_e->mStoplossPoint, m_Price, iter_e->OrderLocalRef);
					WriteMsgToLogList(line);
					...
					char line[200];
					sprintf(line, "%s,Sell Order Stop 0,%.4f,%.4f,%.4f,Ref=%d", mStrategyAndInstance.c_str(), iter_e->OpenOrderTradePrice, iter_e->mStoplossPoint, m_Price, iter_e->OrderLocalRef);
					WriteMsgToLogList(line);
	...
	if (timeRuleOK(pDepthMarketData->updatetime)) {
		...
		if (EnterBuyTrade == 1
			...
			if (OnHandPositionCount <= 0 && (OnHandPositionCount + 1) <= MaxOnHandPositionCount && (TotalOnHandPosition + 1) <= MaxTotalOnHandPosition) {
				...
				openThostOrder.LimitPrice = m_Price + 2 * m_dOneTick;			// 均价挂单
				...
				openThostOrder.VolumeTotal = mStrategyParams.Vol;
				openThostOrder.VolumeTotalOriginal = openThostOrder.VolumeTotal;
				openThostOrder.VolumeTraded = 0;
				openThostOrder.ProfitPrice = openThostOrder.LimitPrice + 100 * m_dOneTick;
		...
		if (EnterSellTrade == 1
			...
			if (OnHandPositionCount <= 0 && (OnHandPositionCount + 1) <= MaxOnHandPositionCount && (TotalOnHandPosition + 1) <= MaxTotalOnHandPosition) {
				...
				openThostOrder.OrderLocalRetReqID = 0;
				openThostOrder.OrderId = -1;
				openThostOrder.LimitPrice = m_Price - 2 * m_dOneTick;;
				openThostOrder.OrigSubmitPrice = openThostOrder.LimitPrice;
...
void StrategyDT::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyDT::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyDT::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
void StrategyDT::AddToRatesX(int tickCount, TickInfo* curTick, BarRateInfo* xBarFile, int* xBarIndex, int xBarTimeSize)
{
	int nYear, nMonth, nDate, nHour, nMin, nSec;
	...
	else
	{
...

		{
			...
			MA1sum += (xBarFile[*xBarIndex].close - xBarFile[*xBarIndex - mStrategyParams.MA1].close);
			PreMA1Avg = MA1Avg;
			MA1Avg = MA1sum / mStrategyParams.MA1;
			MA2sum += (xBarFile[*xBarIndex].close - xBarFile[*xBarIndex - mStrategyParams.MA2].close);
			PreMA2Avg = MA2Avg;
			MA2Avg = MA2sum / mStrategyParams.MA2;
			exitAction(*xBarIndex);
			mBarIndex++;
			(*xBarIndex)++;
		}
		else
...
void StrategyDT::InitVariables()
{
	...
	mBarIndex = mStrategyParams.MA2;
	// 5min
	mBarSize = 60 * mStrategyParams.Ktime;
	LoadHisData(mBarFile);
	//mOpenPrice=67.96;
...
void StrategyDT::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyDT::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.ExchangeID, m_ExchangeID);
...
void StrategyDT::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyDT::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyDT::InitAction()
{
	InitVariables();
}

void StrategyDT::ResetAction()
{
	char line[200];
...
void StrategyDT::exitAction(int i)
{
	char filePath[256] = { 0 };
	globalFuncUtil.getModuleFilePath(filePath);
	strcat(filePath, "\\Data\\");
	...
	CTime mCurrTime = CTime::GetCurrentTime();
	//sprintf(cDate,"%s_%04d%02d%02d",mStrategyAndInstance.c_str(),mCurrTime.GetYear(),mCurrTime.GetMonth(),mCurrTime.GetDay());
	sprintf(cDate, "%s", mStrategyAndInstance.c_str());
	strcat(filePath, cDate);
	strcat(filePath, ".txt");
...
void StrategyDT::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyDT::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
bool StrategyDT::timeRuleOK(char datatime[10])
{
	int nHour, nMin, nSec;
...
void StrategyDT::FlushStrategyInfoToFile() {
...
void StrategyDT::ReleaseDataArray() {
	if (mBarFile != NULL) {
		free(mBarFile);
		mBarFile = NULL;
	}
...
void StrategyDT::InitDataArray() {
	int m_dallocBarNum = 10000;
	mBarFile = (BarRateInfo*)malloc(sizeof(BarRateInfo) * m_dallocBarNum);
	mBarIndex = 0;
}

void StrategyDT::SetParamValue(ParamNode node) {
	bool find = false;
	...
	if (strcmp(node.ParamName, "InstCodeName") == 0) {
		...
		if (strcmp(strExchangeID.c_str(), "CFFEX") == 0 || strcmp(strExchangeID.c_str(), "SHFE") == 0
			|| strcmp(strExchangeID.c_str(), "DCE") == 0 || strcmp(strExchangeID.c_str(), "CZCE") == 0
			|| strcmp(strExchangeID.c_str(), "INE") == 0
			|| strcmp(strExchangeID.c_str(), "SGE") == 0) {
	...
	else if (strcmp(node.ParamName, "MA1") == 0) {
		mStrategyParams.MA1 = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "MA2") == 0) {
		mStrategyParams.MA2 = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "MA1Value") == 0) {
		mStrategyParams.MA1Value = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "MA2Value") == 0) {
		mStrategyParams.MA2Value = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "DT") == 0) {
		mStrategyParams.DT = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "Ktime") == 0) {
		mStrategyParams.Ktime = atoi(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "ProfitPoint1") == 0) {
		mStrategyParams.ProfitPoint1 = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "ProfitPoint2") == 0) {
		mStrategyParams.ProfitPoint2 = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "StoplossPoint") == 0) {
		mStrategyParams.StoplossPoint = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "StoplossPoint1") == 0) {
		mStrategyParams.StoplossPoint1 = atof(node.ParamValue);
	}
	else if (strcmp(node.ParamName, "StoplossPoint2") == 0) {
		mStrategyParams.StoplossPoint2 = atof(node.ParamValue);
	}
	...
	else if (strcmp(node.ParamName, "Vol") == 0) {
		mStrategyParams.Vol = atoi(node.ParamValue);
	}
...
int StrategyDT::CreateStrategyMapOfView() {
	CString strPathFile;
...
int StrategyDT::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
wstring StrategyDT::s2ws(const string& s)
{
...
void StrategyDT::SetInstanceName(char xInstanceName[50])
{
...
void StrategyDT::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyDT::SetStrategyID(char strategyId[50])
{
	strcpy(mStrategyID, strategyId);
}

void StrategyDT::LoadHisData(BarRateInfo* xBarFile)
{
	char filePath[256] = { 0 };
	globalFuncUtil.getModuleFilePath(filePath);
	strcat(filePath, "\\Data\\");
	...
	CTime mNowTime = CTime::GetCurrentTime();
	...
	if (fmddata)
	{
		...
		while (fgets(buff, 1024, fmddata))
		{
			...
			//CTime tm1(mNowTime.GetYear(), mNowTime.GetMonth(), mNowTime.GetDay(),nHour,nMin,nSec);
			int nYear_m, nMonth_m, nDate_m;
			nYear_m = atoi(nDate) / 10000;
			nMonth_m = (atoi(nDate) % 10000) / 100;
			nDate_m = (atoi(nDate) % 10000) % 100;
			CTime tm1(nYear_m, nMonth_m, nDate_m, nHour, nMin, nSec);
			...
			mBarFile[iCounttotal].close = atof(nClose);
			++iCounttotal;
		}
		if (0 != iCount) mAvg = sum / iCount;
		if (iCounttotal >= mStrategyParams.MA2)
		{
			MA1sum = 0;
			MA2sum = 0;
			for (int i = iCounttotal - 1; i > iCounttotal - mStrategyParams.MA2; i--)
			{
				MA2sum += mBarFile[i].close;
				if (i >= iCounttotal - mStrategyParams.MA1)
				{
					MA1sum += mBarFile[i].close;
				}
			}
			MA1Avg = MA1sum / mStrategyParams.MA1;
			MA2Avg = MA2sum / mStrategyParams.MA2;
			...
			DT = max(mYHigh - mYClose, mYClose - mYLow);
		}
		else
		{
			MA1Avg = mStrategyParams.MA1Value;
			MA2Avg = mStrategyParams.MA2Value;
			PreMA1Avg = MA1Avg;
			PreMA2Avg = MA2Avg;
			DT = mStrategyParams.DT;
		}
		char line[200];
		sprintf(line, "%s,yOpen:%.4f,yHigh:%.4f,yLow:%.4f,yClose:%.4f,MA1Avg:%.4f,MA2Avg:%.4f,DT:%.4f", filePath, mYOpen, mYHigh, mYLow, mYClose, MA1Avg, MA2Avg, DT);
		WriteMsgToLogList(line);
		fclose(fmddata);
	}
}

bool StrategyDT::timeRuleForClose(char datadate[10], char datatime[10])
{
...
void StrategyDT::RecoverInstance(char cfgFileName[500])
{
	...
	//memcpy(&DT,header.SpecificArea,sizeof(double));
	//memcpy(&MA1Value,header.SpecificArea+sizeof(double),sizeof(double));
	//memcpy(&MA2Value,header.SpecificArea+sizeof(double)+sizeof(double),sizeof(double));
