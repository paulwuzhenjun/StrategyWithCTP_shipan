		...
		for(closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();){
			...
			if( iter_e->Direction==MORDER_SELL
				&&(iter_e->OrderStatus==MORDER_PART_TRADED||iter_e->OrderStatus==MORDER_ACCEPTED||iter_e->OrderStatus==MORDER_QUEUED)){
					...
					if(iter_e->IsClosePofitOrder&&(m_Price+0.00001>iter_e->ProfitPrice)){
						char line[200];
						sprintf(line,"%s,Buy Order Close,OrderId=%d,price=%.2f",mStrategyAndInstance.c_str(),iter_e->OrderId,iter_e->ProfitPrice);
						WriteMsgToLogList(line);
				...
				&&(iter_e->OrderStatus==MORDER_PART_TRADED||iter_e->OrderStatus==MORDER_ACCEPTED||iter_e->OrderStatus==MORDER_QUEUED)){
					...
					if( iter_e->IsClosePofitOrder&&(m_Price-0.00001<iter_e->ProfitPrice)){
						char line[200];
						sprintf(line,"%s,Sell Order Close,OrderId=%d,price=%.2f",mStrategyAndInstance.c_str(),iter_e->OrderId,iter_e->ProfitPrice);
						WriteMsgToLogList(line);
						MyCloseOrderType newThostOrder;
						newThostOrder.OrderLocalRetReqID=0;
						newThostOrder.CloseOrderSeqNo=0;
...
void StrategyBaseGridOpenCffex::ReqOpenOrderInsert(MyOpenOrderType *pOpenOrder){
	...
	if(ThostTraderAPI||SgitTraderAPI){
		...
		if(ThostTraderAPI)pThostTraderSpi->ReqOrderInsertCffex(&cOrder,m_bCrossTradingDay,false,shmindex,gBuyPosition,gSellPosition);
...
void StrategyBaseGridOpenCffex::ReqCloseOrderInsert(MyCloseOrderType *pCloseOrder,char OpenOrderTime[21]){
	...
	if(ThostTraderAPI||SgitTraderAPI){
		...
		if(ThostTraderAPI)pThostTraderSpi->ReqOrderInsertCffex(&cOrder,m_bCrossTradingDay,CloseToday,shmindex,gBuyPosition,gSellPosition);
...
{
	if(ThostTraderAPI||SgitTraderAPI){
		if(ThostTraderAPI)pThostTraderSpi->ReqOrderDeletePerOrderLocalRef(pOrderLocalRef,m_InstID,pFrontID,pSessionID);
