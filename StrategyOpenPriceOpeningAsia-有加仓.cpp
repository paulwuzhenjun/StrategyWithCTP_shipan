...
void StrategyOpenPriceOpeningAsia::SetParamValue(ParamNode node){
	...
	}else if(strcmp(node.ParamName,"bAsia")==0){
		if(atoi(node.ParamValue)>=1)mStrategyParams.bAsia=true;
		else mStrategyParams.bAsia=false;
...
void StrategyOpenPriceOpeningAsia::OnRtnDepthMarketData(TickInfo *pDepthMarketData)
{
	...
	if(!OpenOrderList.empty()){
		for(openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); ++openorder_it){
			...
			}else if(m_bType1StoplossClose&&openorder_it->MOrderType==2){
				//类型一报单已止损平仓且价格回到开盘价附近则类型二订单可以被撤掉,撤掉后可以重报类型1订单
	...
	if(timeRuleForOpen(pDepthMarketData->updatetime)&&!Type1OrderExist&&!m_bType1StoplossClose&&!Type2StoplossClose){
		...
		for(openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end();openorder_it++){
			...
			if(openorder_it->OrderId==thisTradeRef||(openorder_it->OrderLocalRef==pOrderTradeMsg->OrderLocalRef&&openorder_it->OrderLocalRef>0&&(ThostTraderAPI||SgitTraderAPI))){
				...
				{					
					...
					//报第二个开仓单到交易所
					if(timeRuleForOpen(datatime)&&openorder_it->MOrderType==1&&mOpenTimes<mStrategyParams.LoopTimes
						&&mOpenPrice>0.00001
						&&!mType2OrderExist
		...
		for(closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();closeorder_it++){
			...
			if(closeorder_it->OrderId==thisTradeRef||(closeorder_it->OrderLocalRef==pOrderTradeMsg->OrderLocalRef&&closeorder_it->OrderLocalRef>0&&(ThostTraderAPI||SgitTraderAPI))){
				...
				if(pOrderTradeMsg->Volume==closeorder_it->VolumeTotalOriginal){
					...
					if(closeorder_it->MOrderType==2&&closeorder_it->IsClosePofitOrder&&mOpenTimes<mStrategyParams.LoopTimes
						&&mOpenPrice>0.00001
						&&(OnHandPositionCount+mStrategyParams.OpenVol2)<=MaxOnHandPositionCount
						&&(TotalOnHandPosition+mStrategyParams.OpenVol2)<=MaxTotalOnHandPosition
						&&!mType2OrderExist
							...
							if(closeorder_it->Direction==MORDER_SELL){
								...
								Type2OrderExist=true;
								openThostOrder.mStoplossPoint=mStrategyParams.SecondStoplossPoint*m_dOneTick;
								ReqOpenOrderInsert(&openThostOrder);
							}
					}
...
void StrategyOpenPriceOpeningAsia::OnDisplayLocalCloseOrderList()
{
	...
	for(openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end();openorder_it++){
		...
		strcpy(lcposition.StrategyName,mStrategyName);
