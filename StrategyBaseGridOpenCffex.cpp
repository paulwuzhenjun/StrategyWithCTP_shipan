...
int StrategyBaseGridOpenCffex::MaxOnHandPositionCount = 0;
int StrategyBaseGridOpenCffex::OnHandPositionCount = 0;
...
StrategyBaseGridOpenCffex::StrategyBaseGridOpenCffex(char InstrumentID[30])
{
	iNextOrderRef = 1;
	mTickCount = 0;
	m_dOneTick = 0.01;
	...
	strcpy(mStrategyName, "BaseGridOpenCffex");
	strcpy(mInstanceName, "");
	WriteMsgToLogList("Strategy Init..");
	memset(curTradingDay, 0, sizeof(curTradingDay));
}

StrategyBaseGridOpenCffex::~StrategyBaseGridOpenCffex(void)
{
}

int StrategyBaseGridOpenCffex::GetShmindex()
{
	return shmindex;
}

void StrategyBaseGridOpenCffex::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

void StrategyBaseGridOpenCffex::SetInstanceName(char xInstanceName[50])
{
...
void StrategyBaseGridOpenCffex::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyBaseGridOpenCffex::SetParamValue(ParamNode node) {
	bool find = false;
...
void StrategyBaseGridOpenCffex::InitVariables() {
...
void StrategyBaseGridOpenCffex::ResetStrategy() {
}

void StrategyBaseGridOpenCffex::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
	...
	if (!CloseOrderList.empty() && bOpenFlag) {
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();) {
			...
			if (iter_e->Direction == MORDER_SELL
				...
				if (iter_e->IsClosePofitOrder && (m_Price - iter_e->OpenOrderTradePrice - 0.00001 < iter_e->maxProfit - mStrategyParams.BackPoint * m_dOneTick) && iter_e->MAStop) {
					char line[200];
				...
				if (iter_e->maxProfit + 0.00001 > mStrategyParams.ProfitPoint* m_dOneTick) iter_e->MAStop = true;
				if (iter_e->IsClosePofitOrder && (iter_e->OpenOrderTradePrice - m_Price + 0.00001 < iter_e->maxProfit - mStrategyParams.BackPoint * m_dOneTick) && iter_e->MAStop) {
					char line[200];
	...
	if (nBuyOrderType >= (29 - mStrategyParams.DnGridCount + 1) && nBuyOrderType >= 0 && nBuyOrderType <= 29) {
		if (0 == GridCloseOrderCount[nBuyOrderType]) {
			...
			if (bOpenFlag && mStrategyParams.OpenBuyAllow && (mStrategyParams.BasePrice - 0.00001 > 0) && (mStrategyParams.BasePrice - 0.00001 < mStrategyParams.BuyLimitPrice)
				&& OnHandPositionCount <= MaxOnHandPositionCount && TotalOnHandPosition <= MaxTotalOnHandPosition
				&& bFlag) {
				MyOpenOrderType openThostOrder;
				iNextOrderRef++;
...
void StrategyBaseGridOpenCffex::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBaseGridOpenCffex::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBaseGridOpenCffex::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
void StrategyBaseGridOpenCffex::WideCharToMultiChar(CString str, char* dest_str)
{
...
bool StrategyBaseGridOpenCffex::timeRuleForOpen(char datadate[10], char datatime[10]) {
...
bool StrategyBaseGridOpenCffex::timeRuleForOpenNew(char datadate[10], char datatime[10]) {
	int nHour, nMin, nSec;
	sscanf(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
...
bool StrategyBaseGridOpenCffex::timeRuleForClose(char datadate[10], char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
bool StrategyBaseGridOpenCffex::timeRuleForCancel(char datadate[10], char datatime[10]) {
	int nYear, nMonth, nDate;
	nYear = atoi(datadate) / 10000;
	nMonth = (atoi(datadate) % 10000) / 100;
	nDate = (atoi(datadate) % 10000) % 100;
...
bool StrategyBaseGridOpenCffex::timeRuleForClosePrice(char datadate[10], char datatime[10]) {
	int nHour, nMin, nSec;
	sscanf_s(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
...
void StrategyBaseGridOpenCffex::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyBaseGridOpenCffex::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyBaseGridOpenCffex::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyBaseGridOpenCffex::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyBaseGridOpenCffex::OnDisplayLocalCloseOrderList()
{
	string strIns(m_InstID);
	map<string, Posi>::iterator itor = gPosiMap.find(strIns);
	int nBuyPosition = 0, nSellPosition = 0;
...
void StrategyBaseGridOpenCffex::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyBaseGridOpenCffex::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyBaseGridOpenCffex::RecoverInstance(char cfgFileName[500])
{
...
void StrategyBaseGridOpenCffex::FlushStrategyInfoToFile() {
...
wstring StrategyBaseGridOpenCffex::s2ws(const string& s)
{
...
int StrategyBaseGridOpenCffex::CreateStrategyMapOfView() {
	CString strPathFile;
...
int StrategyBaseGridOpenCffex::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyBaseGridOpenCffex::CloseMap()
{
	//删除对应的cfg文件
...
void StrategyBaseGridOpenCffex::SetCloseLocalOrder(LocalCLForDisplayField* pCLOrder)
{
}

void StrategyBaseGridOpenCffex::SetStrategyID(char strategyId[50])
{
	strcpy(mStrategyID, strategyId);
}

void StrategyBaseGridOpenCffex::InitAction() {
	mTickCount = 0;
	m_bSampleReady = false;
	mOpenRet = false;
	InitVariables();
...
void StrategyBaseGridOpenCffex::WriteBasePrice(char* date, char* time) {
	char line[200];
