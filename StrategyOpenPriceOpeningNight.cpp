...
int StrategyOpenPriceOpeningNight::MaxOnHandPositionCount = 0;
int StrategyOpenPriceOpeningNight::OnHandPositionCount = 0;
...
StrategyOpenPriceOpeningNight::StrategyOpenPriceOpeningNight(char InstrumentID[30]) :m_Price(0)
{
	...
	*/
	...
	strcpy(mStrategyName, "OpenPriceOpeningNight");
...
void StrategyOpenPriceOpeningNight::CloseMap()
{
	//删除对应的cfg文件
...
StrategyOpenPriceOpeningNight::~StrategyOpenPriceOpeningNight(void)
{
}

int StrategyOpenPriceOpeningNight::GetShmindex()
{
	return shmindex;
}

void StrategyOpenPriceOpeningNight::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

wstring StrategyOpenPriceOpeningNight::s2ws(const string& s)
{
...
int StrategyOpenPriceOpeningNight::CreateStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
int StrategyOpenPriceOpeningNight::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyOpenPriceOpeningNight::SetInstanceName(char xInstanceName[50])
{
...
void StrategyOpenPriceOpeningNight::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyOpenPriceOpeningNight::SetParamValue(ParamNode node) {
	bool find = false;
	...
	else if (strcmp(node.ParamName, "NCloseTime") == 0) {
		strcpy(mStrategyParams.NCloseTime, node.ParamValue);
	}
...
void StrategyOpenPriceOpeningNight::InitVariables() {
...
void StrategyOpenPriceOpeningNight::InitAction() {
	InitVariables();
}
void StrategyOpenPriceOpeningNight::ResetStrategy() {
}
//检查消息队列，并将得到的消息转给相应的处理函数
/*
void StrategyOpenPriceOpeningNight::MessageProcess()
{
...
*/
void StrategyOpenPriceOpeningNight::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
...
void StrategyOpenPriceOpeningNight::CheckAndResubmitOutOfDateOrder(char cTickDataDate[11], char cTickUpdateTime[11]) {
...
void StrategyOpenPriceOpeningNight::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyOpenPriceOpeningNight::FlushStrategyInfoToFile() {
...
void StrategyOpenPriceOpeningNight::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyOpenPriceOpeningNight::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
bool StrategyOpenPriceOpeningNight::timeRuleForOpen(char datatime[10]) {
	// 夜盘时间 21:00:00-02:30:00(+1)
	int nHour, nMin, nSec;
	sscanf(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);

	int nOpenHour, nOpenMin, nOpenSec, nNCloseHour, nNCloseMin, nNCloseSec;
	sscanf_s(mStrategyParams.OpenTime, "%d:%d:%d", &nOpenHour, &nOpenMin, &nOpenSec);
	sscanf_s(mStrategyParams.NCloseTime, "%d:%d:%d", &nNCloseHour, &nNCloseMin, &nNCloseSec);

	int nBegin = nOpenHour * 3600 + nOpenMin * 60 + nOpenSec;
	int nEnd = nNCloseHour * 3600 + (nNCloseMin - 1) * 60 + nNCloseSec;
	int nTime = nHour * 3600 + nMin * 60 + nSec;

	if ((nTime >= nBegin && nTime <= 23 * 3600 + 59 * 60 + 59) || (nTime >= 0 && nTime < nEnd)) return true;
	return false;
}

//datadate format: yyyymmdd , datatime formate hh24:mi:ss
//true - must close current order , false - no need to close current order
bool StrategyOpenPriceOpeningNight::timeRuleForClose(char datadate[10], char datatime[10]) {
	...
	int nOpenHour, nOpenMin, nOpenSec, nCloseHour, nCloseMin, nCloseSec, nNCloseHour, nNCloseMin, nNCloseSec;
	sscanf_s(mStrategyParams.OpenTime, "%d:%d:%d", &nOpenHour, &nOpenMin, &nOpenSec);
	sscanf_s(mStrategyParams.CloseTime, "%d:%d:%d", &nCloseHour, &nCloseMin, &nCloseSec);
	sscanf_s(mStrategyParams.NCloseTime, "%d:%d:%d", &nNCloseHour, &nNCloseMin, &nNCloseSec);
	...
	if (mStrategyParams.bNight) {
		CTime tm(nYear, nMonth, nDate, nNCloseHour, nNCloseMin, nNCloseSec);
		time_t NCloseTime = tm.GetTime();

		CTime tm2(nYear, nMonth, nDate, nHour, nMin, nSec);
		time_t currTime = tm2.GetTime();

		if (NCloseTime - currTime > 0 && NCloseTime - currTime < 60) return true;
		return false;
	}
	else {
		// 收盘平仓
...
bool StrategyOpenPriceOpeningNight::timeRuleForCancel(char datadate[10], char datatime[10]) {
	// 夜盘收盘前一分钟撤单
	int nYear, nMonth, nDate;
	nYear = atoi(datadate) / 10000;
	nMonth = (atoi(datadate) % 10000) / 100;
	nDate = (atoi(datadate) % 10000) % 100;
	int nHour, nMin, nSec, nNCloseHour, nNCloseMin, nNCloseSec;
	sscanf(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);
	sscanf(mStrategyParams.NCloseTime, "%d:%d:%d", &nNCloseHour, &nNCloseMin, &nNCloseSec);
...
void StrategyOpenPriceOpeningNight::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyOpenPriceOpeningNight::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyOpenPriceOpeningNight::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyOpenPriceOpeningNight::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyOpenPriceOpeningNight::OnDisplayLocalCloseOrderList()
{
	char log[200];
...
void StrategyOpenPriceOpeningNight::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyOpenPriceOpeningNight::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyOpenPriceOpeningNight::RecoverInstance(char cfgFileName[500])
{
...
void StrategyOpenPriceOpeningNight::SetStrategyID(char strategyId[50])
{
...
void StrategyOpenPriceOpeningNight::FlushPositionProfit(char* dataDate, char* dataTime, int dir, int position)
{
	// 记录文件
	//char line[200];
