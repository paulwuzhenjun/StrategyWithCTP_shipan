...
//递归指标策略
class StrategyOpenPriceOpeningAsia : public CStrategy
{
	struct mParasType
	{
		...
		char OpenEndTime[30];	// 开仓结束时间
		bool bNight;				// 亚盘平仓
		int LoopTimes;
	};

private:
	char  m_InstID[30];  //远期合约类型 m1901
	char  m_CommodityNo[30];
	char m_InstCodeName[30];	// DCE m m1901
...
public:
	StrategyOpenPriceOpeningAsia(char InstrumentID[30]);
	~StrategyOpenPriceOpeningAsia(void);
