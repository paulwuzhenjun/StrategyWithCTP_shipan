...
int StrategyBar::MaxOnHandPositionCount = 0;
int StrategyBar::OnHandPositionCount = 0;
...
StrategyBar::StrategyBar(char InstrumentID[30]) :m_Price(0)
{
...
void StrategyBar::ReleaseDataArray() {
	if (mBarFile != NULL) {
		free(mBarFile);
		free(mBarClose);
		mBarFile = NULL;
...
void StrategyBar::InitDataArray() {
	int m_dallocBarNum = 10000;
	mBarFile = (BarRateInfo*)malloc(sizeof(BarRateInfo) * m_dallocBarNum);
	mBarClose = (double*)malloc(sizeof(double) * m_dallocBarNum);
	mBarIndex = 0;
...
int StrategyBar::GetShmindex()
{
	return shmindex;
}

void StrategyBar::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

wstring StrategyBar::s2ws(const string& s)
{
...
int StrategyBar::CreateStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
int StrategyBar::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyBar::SetInstanceName(char xInstanceName[50])
{
...
void StrategyBar::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyBar::SetParamValue(ParamNode node) {
	bool find = false;
	...
	else if (strcmp(node.ParamName, "n1") == 0) {
		mStrategyParams.n1 = atoi(node.ParamValue);
	}
	...
	else if (strcmp(node.ParamName, "OpenVolMulti") == 0) {
		mStrategyParams.OpenVolMulti = atoi(node.ParamValue);
	}
...
void StrategyBar::InitVariables() {
...
void StrategyBar::InitAction() {
	InitVariables();
}

void StrategyBar::ResetAction() {
	char line[200];
	...
	for (int i = 0; i <= mBarIndex; i++) {
		...
		if (strcmp(mBarFile[i].datatime, "23:30:00") != 0 && strcmp(mBarFile[i].datatime, "15:00:00") != 0) {
...
void StrategyBar::exitAction() {
	...
	for (int i = 0; i <= mBarIndex; i++) {
		//记录行情数据
		char dataline[200];
		sprintf(dataline, "%s,%s,%s,%.2f,%.2f,%.2f,%.2f\n", CTPTradingDay, mBarFile[i].datadate, mBarFile[i].datatime, mBarFile[i].open, mBarFile[i].high, mBarFile[i].low, mBarFile[i].close);
...
/*
void StrategyBar::MessageProcess()
{
...
*/
void StrategyBar::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	...
	mBarClose[mBarIndex] = mBarFile[mBarIndex].close;

	mTickCount++;
	mTodayTickCount++;

	if (lastmBarIndex != mBarIndex && mBarIndex > mStrategyParams.n1) {
		//查找买入信号
		if (mBarFile[mBarIndex - 1].close < mBarFile[mBarIndex - 1 - mStrategyParams.n1].close) {
			EnterBuyTrade = 1;
			char line[200];
	...
	if (!CloseOrderList.empty()) {
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();) {
			...
			if (iter_e->Direction == MORDER_SELL &&
				...
				if ((m_Buy1 + 0.00001) > iter_e->ProfitPrice&& iter_e->IsClosePofitOrder&& m_Buy1 > 0 && mStrategyParams.CloseAllow) {
					char line[200];
				...
				else if ((m_Buy1 - 0.00001) < iter_e->ManualStopPrice && iter_e->IsClosePofitOrder && iter_e->ManualStopPrice > 0.001 && mStrategyParams.CloseAllow) {
					...
					&& EnterSellTrade == 1 && iter_e->MAStop && mStrategyParams.CloseAllow) {
					char line[200];
				...
				else if (iter_e->IsClosePofitOrder && (iter_e->OpenOrderTradePrice - m_Price) >= (iter_e->mStoplossPoint * m_dOneTick - 0.00001 && mStrategyParams.CloseAllow)
				...
				else if ((iter_e->LimitPrice - 0.00001) > m_Sell1&& iter_e->IsStoplessOrder&& iter_e->CanbeCanceled&& mStrategyParams.CloseAllow) {
					char line[200];
				...
				if ((m_Sell1 - 0.00001) < iter_e->ProfitPrice && iter_e->IsClosePofitOrder && m_Sell1 > 0 && mStrategyParams.CloseAllow) {
					char line[200];
				...
				else if ((m_Sell1 + 0.00001) > iter_e->ManualStopPrice&& iter_e->IsClosePofitOrder&& iter_e->ManualStopPrice > 0.001 && mStrategyParams.CloseAllow) {
					//手动止损用,通过手动设置此值,订单实现手动止损
					char line[200];
					...
					&& EnterBuyTrade == 1 && iter_e->MAStop && mStrategyParams.CloseAllow) {
					char line[200];
				...
				else if (iter_e->IsClosePofitOrder && (m_Price - iter_e->OpenOrderTradePrice) >= (iter_e->mStoplossPoint * m_dOneTick - 0.00001) && mStrategyParams.CloseAllow
					//&&timeRuleForClose(pDepthMarketData->datadate,pDepthMarketData->updatetime)
					) {
					char line[200];
				...
				else if (iter_e->LimitPrice < (m_Buy1 - 0.00001) && iter_e->IsStoplessOrder && iter_e->CanbeCanceled && mStrategyParams.CloseAllow) {
					char line[200];
	...
	if (timeRuleForOpen(pDepthMarketData->datadate, pDepthMarketData->updatetime)) {
		...
		if (EnterBuyTrade == 1
			...
			if ((OnHandPositionCount + 1) <= MaxOnHandPositionCount && (TotalOnHandPosition + 1) <= MaxTotalOnHandPosition) {
				//Open Buy Order
				...
				openThostOrder.ProfitPrice = openThostOrder.LimitPrice + 1000 * m_dOneTick;
				...
				openThostOrder.mStoplossPoint = 10;
...
void StrategyBar::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBar::FlushStrategyInfoToFile() {
...
void StrategyBar::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyBar::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
bool StrategyBar::timeRuleForOpen(char datadate[10], char datatime[10]) {
	...
	if (nHour == 0 && nMin < 59) return true;
	return false;
}

//datadate format: yyyymmdd , datatime formate hh24:mi:ss
//true - must close current order , false - no need to close current order
bool StrategyBar::timeRuleForClose(char datadate[10], char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
...
bool StrategyBar::timeRuleOK(char datatime[10]) {
	//int nYear, nMonth, nDate;
	//nYear=atoi(datadate)/10000;
	//nMonth=(atoi(datadate)%10000)/100;
	//nDate=(atoi(datadate)%10000)%100;
	...
	if ((nHour == 23 && nMin == 30 && nSec == 0)
		|| (nHour == 23 && nMin == 0 && nSec == 0)
		|| (nHour == 1 && nMin == 0 && nSec == 0)
		) return false;
	...
	//if(nHour==15&&nMin==0&&nSec==0) return true;
	if (nHour > 20) return true;
	if (nHour < 2) return true;

	return false;
}
void StrategyBar::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyBar::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyBar::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyBar::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyBar::OnDisplayLocalCloseOrderList()
{
	std::list<MyOpenOrderType>::iterator openorder_it;
	for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); openorder_it++) {
	...
	if (mBarIndex > 0) {
		char log[200];
		sprintf(log, "%s,%d", mStrategyAndInstance.c_str(), CloseOrderList.size());
		CString str(log);
		pPubMsg->AddString(str);
	}
}

void StrategyBar::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyBar::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyBar::RecoverInstance(char cfgFileName[500])
{
...
void StrategyBar::AddToRatesX(int tickCount, TickInfo* curTick, BarRateInfo* xBarFile, int* xBarIndex, int xBarTimeSize) {
	int nYear, nMonth, nDate, nHour, nMin, nSec;
...
int StrategyBar::initHstDataFromCSV() {
	string strHstFileName(mStrategyName);
	char xBarSize[20];
	sprintf(xBarSize, "%d", mBarSize);
	...
	//if(mBarIndex>mStrategyParams.NPeriod){
	//	mIndicator.iKDJ(mBarFile,0,mBarIndex+1,14,3,3,3,RSVBuffer,KBuffer,DBuffer,JBuffer);
	//}

	//char dataline[200];
	//for(int i=0;i<=mBarIndex;i++){
	//	sprintf(dataline,"%s,%s,%.4f,%.4f,%.4f,%.4f\n",mBarFile[i].datadate,mBarFile[i].datatime,RSVBuffer[i],KBuffer[i],DBuffer[i],JBuffer[i]);
...
void StrategyBar::AddInToDesRatesX(BarRateInfo* gCurrtRatesX, int gSourceRatesXIndex, BarRateInfo* gDesRatesX, int* gDesRatesXIndex, int gDesRatesXSize) {
...
int StrategyBar::initHstData() {
	...
	FILE* askfp = fopen(strHstFileName.c_str(), "r");
	if (askfp == NULL) { printf("cannot open the file.\n"); return 0; }
	char* askptr;
	char askachBuf[256] = { 0 };
	askptr = fgets(askachBuf, 256, askfp);
	BarRateInfo curBar;
	std::list<BarRateInfo> BarFromFileList;
	char cTradingDay[15];
	while (askptr != NULL) {
		char* column[6], * p = askachBuf;
		int i;
		...
		memset(&curBar, 0, sizeof(BarRateInfo));
		strcpy(cTradingDay, column[0]);
		strcpy(curBar.datadate, column[1]);
		strcpy(curBar.datatime, column[2]);
		curBar.open = atof(column[3]);
		curBar.high = atof(column[4]);
		curBar.low = atof(column[5]);
		curBar.close = atof(column[6]);
		curBar.vol = 1;
		if (strcmp(cTradingDay, CTPTradingDay) == 0)BarFromFileList.push_back(curBar);
		if (BarFromFileList.size() > 6000) {
			BarFromFileList.pop_front();
		}
		askptr = fgets(askachBuf, 256, askfp);
	}
	...
	mBarIndex = 0;
	memset(&curBar, 0, sizeof(BarRateInfo));
	for (std::list<BarRateInfo>::iterator itr = BarFromFileList.begin(); itr != BarFromFileList.end(); ++itr) {
		strcpy(curBar.datadate, itr->datadate);
		strcpy(curBar.datatime, itr->datatime);
		curBar.open = itr->open;
		curBar.high = itr->high;
		curBar.low = itr->low;
		curBar.close = itr->close;
		curBar.vol = itr->vol;

		strcpy(mBarFile[mBarIndex].datadate, curBar.datadate);
		strcpy(mBarFile[mBarIndex].datatime, curBar.datatime);
		mBarFile[mBarIndex].open = curBar.open;
		mBarFile[mBarIndex].high = curBar.high;
		mBarFile[mBarIndex].low = curBar.low;
		mBarFile[mBarIndex].close = curBar.close;
		mBarFile[mBarIndex].vol = curBar.vol;
		mBarClose[mBarIndex] = curBar.close;

		mBarIndex++;
	}

	mTickCount = mBarIndex;
	mBarIndex = mBarIndex - 1;
	if (mBarIndex < 0) {
		printf("no Bar in txt file \n");
		mBarIndex = 0;
	}

	TRACE("init hst data done.\n");
	return 1;
}

void StrategyBar::SetCloseLocalOrder(LocalCLForDisplayField* pCLOrder)
{
	if (CloseOrderList.empty())return;

	std::list<MyCloseOrderType>::iterator closeorder_it;
	for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end(); closeorder_it++) {
		if (closeorder_it->CloseOrderSeqNo == pCLOrder->CloseOrderSeqNo) {
			closeorder_it->ManualStopPrice = pCLOrder->ManualStopPrice;
		}
	}
	FlushStrategyInfoToFile();
}

void StrategyBar::SetStrategyID(char strategyId[50])
{
