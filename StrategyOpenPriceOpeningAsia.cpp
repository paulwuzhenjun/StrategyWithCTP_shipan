...
int StrategyOpenPriceOpeningAsia::MaxOnHandPositionCount = 0;
int StrategyOpenPriceOpeningAsia::OnHandPositionCount = 0;
...
StrategyOpenPriceOpeningAsia::StrategyOpenPriceOpeningAsia(char InstrumentID[30]) :m_Price(0)
{
	...
	*/
	...
	strcpy(mStrategyName, "OpenPriceOpeningAsia");
...
void StrategyOpenPriceOpeningAsia::CloseMap()
{
	//删除对应的cfg文件
...
StrategyOpenPriceOpeningAsia::~StrategyOpenPriceOpeningAsia(void)
{
}

int StrategyOpenPriceOpeningAsia::GetShmindex()
{
	return shmindex;
}

void StrategyOpenPriceOpeningAsia::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

wstring StrategyOpenPriceOpeningAsia::s2ws(const string& s)
{
...
int StrategyOpenPriceOpeningAsia::CreateStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
int StrategyOpenPriceOpeningAsia::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyOpenPriceOpeningAsia::SetInstanceName(char xInstanceName[50])
{
...
void StrategyOpenPriceOpeningAsia::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyOpenPriceOpeningAsia::SetParamValue(ParamNode node) {
	bool find = false;
	...
	if (strcmp(node.ParamName, "InstCodeName") == 0) {
		...
		string strInstID;
		if (strInstCodeName.find("SP") == string::npos)
			strInstID = strInstCodeName.substr(strInstCodeName.find_last_of(" ") + 1, strInstCodeName.length() - strInstCodeName.find_last_of(" "));
		else
			strInstID = strInstCodeName.substr(strInstCodeName.find_last_of(" ") - 2, strInstCodeName.length() - strInstCodeName.find_last_of(" ") + 2);
	...
	else if (strcmp(node.ParamName, "OpenEndTime") == 0) {
		strcpy(mStrategyParams.OpenEndTime, node.ParamValue);
	}
	else if (strcmp(node.ParamName, "bNight") == 0) {
		if (atoi(node.ParamValue) >= 1)mStrategyParams.bNight = true;
		else mStrategyParams.bNight = false;
	}
...
void StrategyOpenPriceOpeningAsia::InitVariables() {
...
void StrategyOpenPriceOpeningAsia::InitAction() {
	mOpenTimes = 0;
	Type1OrderExist = false;
	m_bType1StoplossClose = false;
	mOpenRet = false;
...
void StrategyOpenPriceOpeningAsia::ResetStrategy() {
}
//检查消息队列，并将得到的消息转给相应的处理函数
/*
void StrategyOpenPriceOpeningAsia::MessageProcess()
{
...
*/
void StrategyOpenPriceOpeningAsia::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
	...
	if (nHour_m == nOpenHour && nMin_m >= nOpenMin && !mOpenRet) {
		...
		//mOpenPrice=atof(buff);
		//mOpenPrice=pDepthMarketData->price;
		// 开盘价格引用行情里面
		mOpenPrice = pDepthMarketData->openprice;
		mOpenRet = true;
		char log[200];
	...
	if (curTickTime > tmt_StartTime&& mOpenPrice > 0.00001) {
		EnterBuyTrade = 1;
	}
	...
	if (!OpenOrderList.empty()) {
		for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); ++openorder_it) {
			...
			if (timeRuleForCancel(pDepthMarketData->datadate, pDepthMarketData->updatetime) && openorder_it->OpenOrderCanbeCanceled) {
				//结束时间已到,未成交的开仓单撤掉
				char line[200];
	...
	if (!CloseOrderList.empty()) {
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end(); closeorder_it++) {
			...
			double dOpenPrice = closeorder_it->OpenOrderTradePrice;
			double dVol = closeorder_it->VolumeTotal;
			...
			if (closeorder_it->Direction == MORDER_SELL &&
				...
				if (closeorder_it->CanbeCanceled && closeorder_it->IsClosePofitOrder && (((closeorder_it->OpenOrderTradePrice - m_Price) >= (mStrategyParams.FirstStoplossPoint * m_dOneTick - 0.00001) && closeorder_it->MOrderType == 1))) {
					char line[200];
					...
					&& closeorder_it->IsClosePofitOrder && closeorder_it->CanbeCanceled) {
					char line[200];
					sprintf(line, "%s,Buy Order timeRuleForClose Close,OrderId=%d", mStrategyAndInstance.c_str(), closeorder_it->OrderId);
					WriteMsgToLogList(line);
			...
			if (closeorder_it->Direction == MORDER_BUY &&
				...
				//	closeorder_it->mStoplossPoint=min(0,10-closeorder_it->maxProfit);
				//}
				if (closeorder_it->CanbeCanceled && closeorder_it->IsClosePofitOrder && (((m_Price - closeorder_it->OpenOrderTradePrice) >= (mStrategyParams.FirstStoplossPoint * m_dOneTick - 0.00001) && closeorder_it->MOrderType == 1))) {
					char line[200];
					...
					&& closeorder_it->IsClosePofitOrder && closeorder_it->CanbeCanceled) {
					char line[200];
					sprintf(line, "%s,Sell Order timeRuleForClose Close,OrderId=%d", mStrategyAndInstance.c_str(), closeorder_it->OrderId);
					WriteMsgToLogList(line);
					...
					closeorder_it->NextCloseOrderPrice=m_Buy1+m_dOneTick;
					ReqCloseOrderInsert(&newThostOrder);
	...
	bool x_bType1BuyCloseOrderExist = false;
	bool x_bType1SellCloseOrderExist = false;
		...
		for (corder_itr = CloseOrderList.begin(); corder_itr != CloseOrderList.end(); ++corder_itr) {
			if (corder_itr->MOrderType == 1) {
				if (corder_itr->Direction == MORDER_SELL) {
					x_bType1BuyCloseOrderExist = true;
				}
				else if (corder_itr->Direction == MORDER_BUY) {
					x_bType1SellCloseOrderExist = true;
				}
			}
		}//end for loop
	}

	if (timeRuleForOpen(pDepthMarketData->updatetime) && !m_bType1StoplossClose && nSec_m > nSecond) {
		bool BuyOrderSubmitted = false; bool SellOrderSubmitted = false;
		if (EnterBuyTrade == 1
			&& mOpenTimes < mStrategyParams.LoopTimes
			&& !x_bType1BuyOrderExist && !x_bType1SellCloseOrderExist && !x_bType1BuyCloseOrderExist
		...
		if (EnterSellTrade == 1
			&& mOpenTimes < mStrategyParams.LoopTimes
			&& !x_bType1SellOrderExist && !x_bType1BuyCloseOrderExist && !x_bType1SellCloseOrderExist
...
void StrategyOpenPriceOpeningAsia::CheckAndResubmitOutOfDateOrder(char cTickDataDate[11], char cTickUpdateTime[11]) {
...
void StrategyOpenPriceOpeningAsia::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
	int thisOrderId = (pOrderTradeMsg->OrderSysId);
	//UpdateOrderIdToOrderList(thisOrderId);
	char line[200];
	sprintf(line, "%s,OnRtnOrder,OrderId=%d,Status=%c,VolRemain=%d,mPosition=%d", mStrategyAndInstance.c_str(), thisOrderId, pOrderTradeMsg->OrderStatus, pOrderTradeMsg->VolumeTotal, mPosition);
	WriteMsgToLogList(line);
	...
	if (!OpenOrderList.empty()) {
		for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); openorder_it++) {
			...
			if (openorder_it->OrderId == thisOrderId || (openorder_it->OrderLocalRef == pOrderTradeMsg->OrderLocalRef && openorder_it->OrderLocalRef > 0 && (ThostTraderAPI || SgitTraderAPI))) {
				...
				if (pOrderTradeMsg->OrderStatus == MORDER_PART_CANCELLED || pOrderTradeMsg->OrderStatus == MORDER_CANCELLED) {
					...
					if (openorder_it->MOrderType == 1)mOpenTimes--;
		...
		char line2[200];
		sprintf(line2, "%s,OnRtnOrder,mPosition=%d", mStrategyAndInstance.c_str(), mPosition);
		WriteMsgToLogList(line2);
	}
...
void StrategyOpenPriceOpeningAsia::FlushStrategyInfoToFile() {
	...
	memcpy(header.SpecificArea + sizeof(double), &mPosition, sizeof(int));

	// ---save closeprofit
	memcpy(header.SpecificArea + sizeof(double) + sizeof(int), &mCloseProfit, sizeof(double));
	// End
...
void StrategyOpenPriceOpeningAsia::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
	...
	if (iter == matchnomap.end()) {
		...
		if (!OpenOrderList.empty()) {
			for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); openorder_it++) {
				...
				if (openorder_it->OrderId == thisTradeRef || (openorder_it->OrderLocalRef == pOrderTradeMsg->OrderLocalRef && openorder_it->OrderLocalRef > 0 && (ThostTraderAPI || SgitTraderAPI))) {
					...
					mPosition += pOrderTradeMsg->Volume;
			...
			for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end(); closeorder_it++) {
				...
				if (closeorder_it->OrderId == thisTradeRef || (closeorder_it->OrderLocalRef == pOrderTradeMsg->OrderLocalRef && closeorder_it->OrderLocalRef > 0 && (ThostTraderAPI || SgitTraderAPI))) {
					...
					if (pOrderTradeMsg->Volume == closeorder_it->VolumeTotalOriginal) {
						//第一单止盈成交,继续报第一单
						if (timeRuleForOpen(datatime) && closeorder_it->MOrderType == 1 && closeorder_it->IsClosePofitOrder && mOpenTimes < mStrategyParams.LoopTimes
							&& mOpenPrice>0.00001
							&& (OnHandPositionCount + mStrategyParams.OpenVol1) <= MaxOnHandPositionCount
							&& (TotalOnHandPosition + mStrategyParams.OpenVol1) <= MaxTotalOnHandPosition
							&& mPosition <= 0) {
							if (closeorder_it->Direction == MORDER_SELL) {
								char line[200];
								sprintf(line, "%s,Open Type 1 Order,Buy,COrderId=%d", mStrategyAndInstance.c_str(), closeorder_it->OrderId);
								WriteMsgToLogList(line);
								...
								char line[200];
								sprintf(line, "%s,Open Type 1 Order,Sell,COrderId=%d", mStrategyAndInstance.c_str(), closeorder_it->OrderId);
								WriteMsgToLogList(line);
		...
		char line2[200];
		sprintf(line2, "%s,OnRtnTrade,OnHandPositionCount=%d", mStrategyAndInstance.c_str(), OnHandPositionCount);
		WriteMsgToLogList(line2);

		if (tradedirection != -1) {
			TradeLogType trade;
			strcpy(trade.AccountID, LoginUserTDCTP);
...
void StrategyOpenPriceOpeningAsia::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
bool StrategyOpenPriceOpeningAsia::timeRuleForOpen(char datatime[10]) {
	// 夜盘时间 21:00:00-23:30:00
	int nHour, nMin, nSec;
	sscanf(datatime, "%d:%d:%d", &nHour, &nMin, &nSec);

	int nOpenHour, nOpenMin, nOpenSec, nEndHour, nEndMin, nEndSec;
	sscanf_s(mStrategyParams.OpenTime, "%d:%d:%d", &nOpenHour, &nOpenMin, &nOpenSec);
	sscanf_s(mStrategyParams.OpenEndTime, "%d:%d:%d", &nEndHour, &nEndMin, &nEndSec);

	int nBegin = nOpenHour * 3600 + nOpenMin * 60 + nOpenSec;
	int nEnd = nEndHour * 3600 + (nEndMin - 1) * 60 + nEndSec;
...
bool StrategyOpenPriceOpeningAsia::timeRuleForClose(char datadate[10], char datatime[10]) {
	...
	int nOpenHour, nOpenMin, nOpenSec, nCloseHour, nCloseMin, nCloseSec, nEndHour, nEndMin, nEndSec;
	sscanf_s(mStrategyParams.OpenTime, "%d:%d:%d", &nOpenHour, &nOpenMin, &nOpenSec);
	sscanf_s(mStrategyParams.CloseTime, "%d:%d:%d", &nCloseHour, &nCloseMin, &nCloseSec);
	sscanf_s(mStrategyParams.OpenEndTime, "%d:%d:%d", &nEndHour, &nEndMin, &nEndSec);
	...
	int nTime = nHour * 3600 + nMin * 60 + nSec;
	// 夜盘平仓
	if (mStrategyParams.bNight) {
		if (nTime >= nEndHour * 3600 + (nEndMin - 1) * 60 + nEndSec) return true;
		return false;
	}
	else {
		// 收盘前一分钟平仓
		...
		if (closeTime - currTime > 0 && closeTime - currTime <= 60) return true;
		return false;
	}
}

bool StrategyOpenPriceOpeningAsia::timeRuleForCancel(char datadate[10], char datatime[10]) {
	// 开仓截至前一分钟撤单
	int nYear, nMonth, nDate;
	nYear = atoi(datadate) / 10000;
	nMonth = (atoi(datadate) % 10000) / 100;
	...
	sscanf(mStrategyParams.OpenEndTime, "%d:%d:%d", &nHour2, &nMin2, &nSec2);
	CTime tm(nYear, nMonth, nDate, nHour2, nMin2, nSec2);
	time_t cancelTime = tm.GetTime();

	CTime tm2(nYear, nMonth, nDate, nHour, nMin, nSec);
	time_t currTime = tm2.GetTime();

	if (cancelTime - currTime > 0 && cancelTime - currTime < 60) return true;

	return false;
}

void StrategyOpenPriceOpeningAsia::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyOpenPriceOpeningAsia::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyOpenPriceOpeningAsia::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyOpenPriceOpeningAsia::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyOpenPriceOpeningAsia::OnDisplayLocalCloseOrderList()
{
	char log[200];
	sprintf(log, "%s,策略开盘价=%.5f,开盘时间:%s,mPosition=%d", mStrategyAndInstance.c_str(), mOpenPrice, mStrategyParams.OpenTime, mPosition);
	CString str(log);
	pPubMsg->AddString(str);
...
void StrategyOpenPriceOpeningAsia::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyOpenPriceOpeningAsia::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyOpenPriceOpeningAsia::RecoverInstance(char cfgFileName[500])
{
	...
	memcpy(&mPosition, header.SpecificArea + sizeof(double), sizeof(int));
	//	strcpy(header.StartTime,mStrategyParams.StartTime);
	//	strcpy(header.EndTime,mStrategyParams.EndTime);

		// recover
	memcpy(&mCloseProfit, header.SpecificArea + sizeof(double) + sizeof(int), sizeof(double));
	if (mCloseProfit + 0.0001 < 0) mCloseProfit = 0;
	// End
...
void StrategyOpenPriceOpeningAsia::SetStrategyID(char strategyId[50])
{
...
void StrategyOpenPriceOpeningAsia::FlushPositionProfit(char* dataDate, char* dataTime, int dir, int position)
{
	// 记录文件
	//char line[200];
	//sprintf(line,"%s,%s,%s,%.2f,%.2f",mStrategyID,dataDate,dataTime,mCloseProfit,mPositionProfit);
	//Message logMsg;
	//logMsg.type=PROFIT_LOG;
	//logMsg.AddData(line,0,sizeof(line));
	//LogMessageList.AddTail(logMsg);

	//ProfitLogType profit;
	//strcpy(profit.StrategyID,mStrategyID);
	//strcpy(profit.InstanceName,mInstanceName);
	//strcpy(profit.Datadate,dataDate);
	//strcpy(profit.Datatime,dataTime);
	//strcpy(profit.instName,mStrategyParams.InstCodeName);
	//profit.CloseProfit=mCloseProfit;
	//profit.PositionProfit=mPositionProfit;
	//profit.Position=position;
	//if(EsunAPIUsed) strcpy(profit.AccountName,LoginUserTDEsun);
	//else if(CTPAPIUsed) strcpy(profit.AccountName,LoginUserTDCTP);

	//Message logMsg;
	//logMsg.type=PROFIT_LOG;
	//logMsg.AddData(&profit,0,sizeof(ProfitLogType));
	//LogMessageList.AddTail(logMsg);
