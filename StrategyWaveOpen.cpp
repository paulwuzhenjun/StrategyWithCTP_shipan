...
int StrategyWaveOpen::MaxOnHandPositionCount = 0;
int StrategyWaveOpen::OnHandPositionCount = 0;
...
StrategyWaveOpen::StrategyWaveOpen(char InstrumentID[30]) :m_Price(0)
{
	...
	strcpy(mStrategyName, "WaveOpen");
	strcpy(mInstanceName, "");
	WriteMsgToLogList("Strategy Init..");
}

StrategyWaveOpen::~StrategyWaveOpen(void)
{
	//fclose(foutbardata);
}

int StrategyWaveOpen::GetShmindex()
{
	return shmindex;
}

void StrategyWaveOpen::SetShmindex(int xshmindex)
{
	shmindex = xshmindex;
}

void StrategyWaveOpen::SetInstanceName(char xInstanceName[50])
{
...
void StrategyWaveOpen::GetInstanceName(char* xInstanceName) {
	strcpy(xInstanceName, mInstanceName);
}

void StrategyWaveOpen::SetParamValue(ParamNode node) {
	bool find = false;
...
void StrategyWaveOpen::InitVariables() {
...
void StrategyWaveOpen::ResetStrategy() {
}
//检查消息队列，并将得到的消息转给相应的处理函数
/*
void StrategyWaveOpen::MessageProcess()
{
...
*/
void StrategyWaveOpen::OnRtnDepthMarketData(TickInfo* pDepthMarketData)
{
	///读取行情价格，设置当前本地的当前价格
	...
	if (!CloseOrderList.empty()) {
		...
		for (closeorder_it = CloseOrderList.begin(); closeorder_it != CloseOrderList.end();) {
			...
			if (iter_e->Direction == MORDER_SELL &&
				...
				if (iter_e->IsClosePofitOrder && (iter_e->OpenOrderTradePrice - m_Price) >= (mStrategyParams.StoplossPoint * m_dOneTick - 0.00001)) {
					char line[200];
			...
			if (iter_e->Direction == MORDER_BUY &&
				...
				//	iter_e->MAStop=true;
				//}
				if (iter_e->IsClosePofitOrder && (m_Price - iter_e->OpenOrderTradePrice) >= (mStrategyParams.StoplossPoint * m_dOneTick - 0.00001)) {
					char line[200];
...
/*
void StrategyWaveOpen::DeleteErrorOrderIdFromOrderList(){
...
void StrategyWaveOpen::OnRtnOrder(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyWaveOpen::OnRtnTrade(OrderTradeMsg* pOrderTradeMsg)
{
...
void StrategyWaveOpen::OnRspOrderInsert(ShmRspOrderInsert* pRspOrderInsert)
{
...
void StrategyWaveOpen::WideCharToMultiChar(CString str, char* dest_str)
{
...
bool StrategyWaveOpen::timeRuleForOpen(char datadate[10], char datatime[10]) {
...
bool StrategyWaveOpen::timeRuleForClose(char datadate[10], char datatime[10]) {
	int nYear, nMonth, nDate;
	nYear = atoi(datadate) / 10000;
	nMonth = (atoi(datadate) % 10000) / 100;
	nDate = (atoi(datadate) % 10000) % 100;
...
void StrategyWaveOpen::ReqOpenOrderInsert(MyOpenOrderType* pOpenOrder) {
...
void StrategyWaveOpen::ReqCloseOrderInsert(MyCloseOrderType* pCloseOrder, char OpenOrderTime[21]) {
	InsertOrderField cOrder;
	strcpy(cOrder.ClientNo, "");
	strcpy(cOrder.CommodityNo, m_CommodityNo);
	strcpy(cOrder.InstrumentID, m_InstID);
...
void StrategyWaveOpen::ReqOrderDelete(int pOrderId, int pOrderLocalRef, int pFrontID, int pSessionID)
{
	if (ThostTraderAPI || SgitTraderAPI) {
...
void StrategyWaveOpen::WriteMsgToLogList(char logline[200])
{
	Message logMsg;
...
void StrategyWaveOpen::OnDisplayLocalCloseOrderList()
{
	std::list<MyOpenOrderType>::iterator openorder_it;
	for (openorder_it = OpenOrderList.begin(); openorder_it != OpenOrderList.end(); openorder_it++) {
...
void StrategyWaveOpen::AddtoTipMsgListBox(char msgline[200])
{
...
void StrategyWaveOpen::DisplayTradeOnScreen(OrderTradeMsg* pOrderTradeMsg, int mDirection, int mOpenOrClose, int mCloseProfitOrNot)
{
	WaitForSingleObject(MainScreenFreshSem, INFINITE);
...
void StrategyWaveOpen::RecoverInstance(char cfgFileName[500])
{
	...
	char log[200];
	sprintf(log, "%s,StrategyWaveOpen,重启恢复,当前时间=%s", mStrategyAndInstance.c_str(), c_str_mCurrTime);
	CString str(log);
	pPubMsg->AddString(str);
	WriteMsgToLogList(log);
...
void StrategyWaveOpen::FlushStrategyInfoToFile() {
...
wstring StrategyWaveOpen::s2ws(const string& s)
{
...
int StrategyWaveOpen::CreateStrategyMapOfView() {
	CString strPathFile;
...
int StrategyWaveOpen::OpenStrategyMapOfView() {
	CString strPathFile;
	::GetModuleFileName(NULL, strPathFile.GetBuffer(MAX_PATH), MAX_PATH);
	strPathFile.ReleaseBuffer();
	strPathFile = strPathFile.Left(strPathFile.ReverseFind(_T('\\')));
...
void StrategyWaveOpen::CloseMap()
{
	//删除对应的cfg文件
...
void StrategyWaveOpen::SetCloseLocalOrder(LocalCLForDisplayField* pCLOrder)
{
}

void StrategyWaveOpen::SetStrategyID(char strategyId[50])
{
